...
def loop(m, n):
    return max(horizontal(m, n), vertical(m, n), diagonal_up(m, n), diagonal_down(m, n))

def horizontal(m, n):
    max_val = 0
    for r in range(m.shape[0]):
        for c in range(m.shape[1]-n):
            val = 1
            for i in range(n):
                val *= m[r, c+i]
            if val > max_val:
