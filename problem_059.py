...
def calc():
    cipher = map(int, open('data/problem_059.txt', 'r').read().rstrip('\n').split(','))
    parts = [[cipher[i] for i in range(len(cipher)) if i%3 == j] for j in range(3)]
    keys = [int(mode(part)[0][0])^ord(' ') for part in parts]
    total = sum([cipher[i]^keys[(i%3)] for i in range(0, len(cipher))])
    text = ''.join([chr(cipher[i]^keys[(i%3)]) for i in range(0, len(cipher))])
    return total, text


if __name__ == '__main__':
    print calc()
    print timeit.Timer('problem_059.calc()', 'import problem_059').timeit(1)
